...
export class AppService {
    ...
    public async signup({email, username, password}: CreateUserDto) {
        ...
        const checkUsername = await this.usersRepository.findOne({where: {username}});
    ...
    private generateJWT(options: { email: string, refresh: boolean }) {
        const access = this.jwtService.sign(
            {email: options.email},
            {expiresIn: this.configService.get("ACCESS_EXPIRES_IN"), secret: this.configService.get("JWT_SECRET")},
        );
        if (options.refresh) {
            const refresh = this.jwtService.sign(
                {email: options.email},
                {expiresIn: this.configService.get("REFRESH_EXPIRES_IN"), secret: this.configService.get("JWT_SECRET")},
            );
